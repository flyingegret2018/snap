...
loop_rs_1: for (i = 0; i < num; i++) {
	#pragma HLS PIPELINE
		...
		//loop_rs_2: for (k = 0; k < size_scatter/BPERDW; k++) {
		//	blockram[i*size_scatter/BPERDW + k] = (din_gmem+idx_ram[i])[k];
		//}
	}
}
//----------------------------------------------------------------------
		...
		action_reg *act_reg)
{
	...
	if((mode & 0x1) == 0) {
		...
		//	block_ram[i] = din_gmem[idx_ram[i/(size_scatter/BPERDW)] + i%num];

			//#pragma HLS PIPELINE rewind
			//#pragma HLS UNROLL factor=8
			//temp_idx = as_ram[i/(BPERDW/8)](64*j + 63, 64*j) >> ADDR_RIGHT_SHIFT;
			////memcpy(blockram + i*size_scatter/BPERDW, (snap_membus_t *)(din_gmem + temp_idx), size_scatter);

			////Assume size_scatter is multiplies of BPERDW
			//for (k = 0; k < size_scatter/BPERDW; k++) {
			//	blockram[i*size_scatter/BPERDW + k] = din_gmem[temp_idx + k];
			//}
